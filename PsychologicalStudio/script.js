document.addEventListener('DOMContentLoaded', function() {
    ...
    for (let group = 0; group < 10; group++) {
        ...
        volumeLabel.textContent = `${group + 1}: `; // Changed from group to group + 1
    ...
    longLoopButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Get long loop length from dataset and multiply by 2
            const baseLoopLength = parseInt(this.dataset.longLoop);
            
            // Validate the parsed value
            if (isNaN(baseLoopLength) || baseLoopLength <= 0) {
                console.error('Invalid long loop length value:', this.dataset.longLoop);
                return;
            }
            
            // Set long loop length to double the value
            const newLongLoopLength = baseLoopLength * 4;
            ...
            console.log(`Long sample loop length set to ${longLoopLength} bars (double of ${baseLoopLength})`);
        });
    });
    ...
    function scheduleSampleForNextBar(group) {
        ...
        const nextBarBoundary = Math.ceil(currentTime / barDuration) * barDuration;
        
        // Set a timeout to play the sample at the right time
        const timeUntilNextBar = (nextBarBoundary - currentTime) * 1000;
    ...
    playButton.addEventListener('click', function() {
        if (!isPlaying) {
            ...
            // Set the next bar time to the next bar boundary
            nextBarTime = Math.ceil(audioContext.currentTime / barDuration) * barDuration;
