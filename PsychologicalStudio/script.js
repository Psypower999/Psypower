document.addEventListener("DOMContentLoaded", function () {
    ...
    function initializeEffectsForSample(sampleNumber) {
        ...
        // Initialize LFO for main sample
    ...
    function showEffectsPopup(sampleNumber, button) {
        ...
        // Set LFO values
        document.getElementById("lfo-target").value = temporaryEffects.lfo.target || "pitch";
        document.getElementById("lfo-waveform").value = temporaryEffects.lfo.waveform || "sine";
        document.getElementById("lfo-rate").value = temporaryEffects.lfo.rate || 1.0;
        document.getElementById("lfo-rate-value").textContent = (temporaryEffects.lfo.rate || 1.0).toFixed(1);
        document.getElementById("lfo-depth").value = temporaryEffects.lfo.depth || 0;
        document.getElementById("lfo-depth-value").textContent = `${temporaryEffects.lfo.depth || 0}%`;
        ...
        setTimeout(() => {
            ...
            document.getElementById("lfo-target").addEventListener("change", updateLFOInRealTime);
            document.getElementById("lfo-waveform").addEventListener("change", updateLFOInRealTime);
            document.getElementById("lfo-rate").addEventListener("input", updateLFOInRealTime);
            document.getElementById("lfo-depth").addEventListener("input", updateLFOInRealTime);
    ...
    function initializeLFOConnection(sampleNumber) {
        ...
        if (target === "pitch") {
            // For pitch modulation, we need to use a more complex approach
            // since we can't directly connect to playbackRate
            sample.lfoTarget = "pitch";
            
            // Create a script processor to modulate the pitch
            if (!sample.lfoPitchProcessor) {
                sample.lfoPitchProcessor = audioContext.createScriptProcessor(256, 1, 1);
                sample.lfoPitchProcessor.onaudioprocess = function(e) {
                    if (sample.source && sample.lfoTarget === "pitch") {
                        const basePlaybackRate = sample.effects ? 
                            (sample.effects.speed || 1) * (sample.effects.individualTempo || 1) : 1;
                        const lfoValue = sample.lfoGainNode.gain.value;
                        const modulation = Math.sin(audioContext.currentTime * sample.lfoNode.frequency.value * Math.PI * 2) * lfoValue * 0.2;
                        sample.source.playbackRate.value = basePlaybackRate * (1 + modulation);
                    }
                };
                
                sample.lfoGainNode.connect(sample.lfoPitchProcessor);
                sample.lfoPitchProcessor.connect(sample.lfoGainNode);
            }
        } else if (target === "filter" && sample.eqLowNode) {
            sample.lfoGainNode.connect(sample.eqLowNode.frequency);
            sample.lfoTarget = "filter";
        } else if (target === "volume" && sample.gainNode) {
            sample.lfoGainNode.connect(sample.gainNode.gain);
    ...
    function updateSampleEffects(sampleNumber) {
        ...
        if (effects.lfo && sample.lfoNode) {
            ...
            // Connect LFO to target based on selection
    ...
    function playSampleAtTime(sampleNumber, startTime) {
        ...
        if (effects.pianoRoll && effects.pianoRoll.notes.length > 0) {
            ...
            sortedNotes.forEach(note => {
                ...
                if (pianoRoll.soundSource === "piano") {
                    playPianoNoteForSample(note.row, noteTime, noteDuration, gainNode, sampleNumber);
                } else if (pianoRoll.soundSource === "synth") {
                    playSynthNoteForSample(note.row, noteTime, noteDuration, gainNode, sampleNumber);
                } else if (pianoRoll.soundSource === "strings") {
                    playStringsNoteForSample(note.row, noteTime, noteDuration, gainNode, sampleNumber);
                } else if (pianoRoll.soundSource === "bass") {
                    playBassNoteForSample(note.row, noteTime, noteDuration, gainNode, sampleNumber);
                } else if (pianoRoll.soundSource === "lead") {
                    playLeadNoteForSample(note.row, noteTime, noteDuration, gainNode, sampleNumber);
                } else if (pianoRoll.soundSource === "pad") {
                    playPadNoteForSample(note.row, noteTime, noteDuration, gainNode, sampleNumber);
                } else if (pianoRoll.soundSource === "pluck") {
                    playPluckNoteForSample(note.row, noteTime, noteDuration, gainNode, sampleNumber);
                } else {
                    playSampleNoteForSample(note.row, noteTime, noteDuration, gainNode, sampleNumber, pianoRoll.sampleRange);
                }
            });
        } else {
        ...
        if (effects.lfo && effects.lfo.depth > 0) {
            ...
            if (target === "pitch") {
                ...
                // Create a script processor to modulate the pitch
                if (!currentPlaying[sampleNumber].lfoPitchProcessor) {
                    currentPlaying[sampleNumber].lfoPitchProcessor = audioContext.createScriptProcessor(256, 1, 1);
                    currentPlaying[sampleNumber].lfoPitchProcessor.onaudioprocess = function(e) {
                        if (currentPlaying[sampleNumber].source && currentPlaying[sampleNumber].lfoTarget === "pitch") {
                            const basePlaybackRate = currentPlaying[sampleNumber].effects ? 
                                (currentPlaying[sampleNumber].effects.speed || 1) * (currentPlaying[sampleNumber].effects.individualTempo || 1) : 1;
                            const lfoValue = currentPlaying[sampleNumber].lfoGainNode.gain.value;
                            const modulation = Math.sin(audioContext.currentTime * currentPlaying[sampleNumber].lfoNode.frequency.value * Math.PI * 2) * lfoValue * 0.2;
                            currentPlaying[sampleNumber].source.playbackRate.value = basePlaybackRate * (1 + modulation);
                        }
                    };
                    
                    currentPlaying[sampleNumber].lfoGainNode.connect(currentPlaying[sampleNumber].lfoPitchProcessor);
                    currentPlaying[sampleNumber].lfoPitchProcessor.connect(currentPlaying[sampleNumber].lfoGainNode);
                }
            } else if (target === "filter" && currentPlaying[sampleNumber].eqLowNode) {
                currentPlaying[sampleNumber].lfoGainNode.connect(currentPlaying[sampleNumber].eqLowNode.frequency);
                currentPlaying[sampleNumber].lfoTarget = "filter";
            } else if (target === "volume" && currentPlaying[sampleNumber].gainNode) {
                currentPlaying[sampleNumber].lfoGainNode.connect(currentPlaying[sampleNumber].gainNode.gain);
