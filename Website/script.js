...
async function loadAlbumSongs(albumName) {
    ...
    const p = (async () => {
        try {
            // Use a more reliable way to get the manifest URL
            const manifestUrl = new URL('Website/songs/manifest.json', window.location.origin + window.location.pathname).href;
...
async function renderSongSearchResults(query) {
    ...
    try {
        ...
        for (const r of results) {
            const cover = new URL(`Website/songs/${r.albumName}/cover.jpg`, window.location.origin + window.location.pathname).href;
            fragments.push(
        ...
        Array.from(ul.querySelectorAll('li[data-album][data-track]')).forEach(li => {
            li.addEventListener('click', () => {
                ...
                currFolder = new URL(`Website/songs/${albumName}`, window.location.origin + window.location.pathname).href;
...
async function getSongs(albumName) {
    ...
    if (songs.length > 0) {
        for (let i = 0; i < songs.length; i++) {
            ...
            const cover = new URL(`${currFolder}/cover.jpg`, window.location.origin + window.location.pathname).href;
            songUL.innerHTML = songUL.innerHTML + `<li data-index="${i}"><img width="40" height="40" src="${cover}" alt="Album cover" style="border-radius:4px;object-fit:cover;">
                                <div class="info">
                                    <div>${cleanName}</div>
                                    <div>Psypower</div>
                                </div>
    ...
    if (!pause) {
        currentSong.play()
            .then(() => {
                ...
                play.src = new URL('Website/img/pause.svg', window.location.origin + window.location.pathname).href;
            })
            .catch(error => {
...
async function displayAlbums() {
    ...
    try {
        ...
        for (const album of manifest.albums) {
            const cover = new URL(`Website/songs/${album.folder}/cover.jpg`, window.location.origin + window.location.pathname).href;
...
async function main() {
    ...
    play.addEventListener("click", () => {
        if (currentSong.paused) {
            ...
            currentSong.pause()
            play.src = new URL('Website/img/play.svg', window.location.origin + window.location.pathname).href;
        }
    })
